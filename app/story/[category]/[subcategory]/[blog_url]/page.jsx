          ...
          <div id='main-content' className='mt-10'>
            {content.blocks.slice(2).map((block, index) => {
              switch (block.type) {
                case 'paragraph':
                  return (
                    <div key={index} className="mb-8 text-[18px] font-[Georgia] font-normal">
                      {parse(block.data.text, {
                        replace: (domNode) => {
                          ...
                          if (domNode.name === 'i') {
                            return <i className="italic">{domToReact(domNode.children)}</i>;
                          }
                ...
                default:
                  return <p key={index} className='text-[18px] font-normal font-[Georgia] mb-8'>Unsupported block type: {block.type}</p>;
              }
            })}
          </div>
        </div>
